...
* As you can't interact with social media, use the `ask_user_to` tool to ask the user to do actions you can't such as tweet.  
* The current date is {datetime.today().strftime('%A, %B %-d, %Y')}.
</SYSTEM_CAPABILITY>

<IMPORTANT>
* When using Chrome or other applications, if any startup wizards or prompts appear, **IGNORE THEM**. Do not interact with them. Instead, click on the address bar or the area where you can enter commands or URLs, and proceed with your task.
...
* You will be pre-authorized on the Chrome browser to use any tools or sites requested so use Chrome for any browser requests. 
</IMPORTANT>"""
...
async def sampling_loop(
    *,
    ...
    tool_collection = ToolCollection(
        ComputerTool(), BashTool(), EditTool(), AskUserTo(), Resources()
    )
